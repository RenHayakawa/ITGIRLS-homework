...
for (let superhero of superheroes) {
    ...
    let nameStr = name.replace(/\//g, " / ");

    let occupation = superhero.occupation;
    let occupationStr = occupation.join();
    let occupationStrFinal = occupationStr.replace(/\,/g, ", ");

    let friends = superhero.friends;
    let friendsStr = friends.join();
    let friendsStrFinal = friendsStr.replace(/\,/g, ", ");

    let superpowers = superhero.superpowers;
    let superpowersStr = superpowers.join();
    let superpowersStrFinal = superpowersStr.replace(/\,/g, ", ");

    container += `<div class="heroesCards">
        <h2>${nameStr}</h2>
        <img src="${superhero.image}" width= 80%>
        <p><span>Вселенная:</span> ${superhero.universe}</p>
        <p><span>Альтер эго:</span> ${superhero.alterEgo}</p>
        <p><span>Род деятельности:</span> ${occupationStrFinal}</p>
        <p><span>Друзья:</span> ${friendsStrFinal}</p>
        <p><span>Суперсилы:</span> ${superpowersStrFinal}</p>
        <p><span>Подробнее:</span> ${superhero.details}</p>
        <span>Оцени героя/героиню: </span><input id="scores" type="number" min=0 max=10><span> / 10</span>
        </div>`
}
